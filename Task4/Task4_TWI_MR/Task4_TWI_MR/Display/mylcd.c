...
uint8_t lcd_read_data(uint8_t chip)
{
  ...
  _delay_us(2);				// wait a moment until data is (hopefully) stable
  if (invert!=0) 
...
{
  #ifndef LCD_NO_RES
    LCD_CMD_DDR |= (1<<CD) | (1<<RW) | (1<<EN) | (1<<CS1) | (1<<CS2) | (1<<RES);
  #else
...
// ---------------------------------
uint8_t font_char_width(const uint8_t* font,uint8_t chr)
{
...
// ----------------------------------------
uint8_t font_char_present(const uint8_t* font,uint8_t chr)
{
...
// ---------------------------------------
void lcd_puts_p(const uint8_t* font,char* PROGMEM string)
{
